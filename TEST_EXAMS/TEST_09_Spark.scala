...
matchesNeeded.coalesce(1).write.csv("/home/ubuntu/finalScala/matchesNeeded")
...
stats
      ...
      // ------------------ Matches Data ------------------
      val matches1Path = s"$path/matchesID1.json"
      val matches13Path = s"$path/matchesID13.json"
      val matches17Path = s"$path/matchesID17.json"
      val matches24Path = s"$path/matchesID24.json"
      ...
      // Filter Date 2021      
      val matchesDf02 = matchesDf01.filter(col("date").contains("2021")).drop(col("date"))
      // Filter Team ids 
      val dfm01 = matchesDf02.filter(col("home_team_id").isin("1", "17", "13", "24") || col("visitor_team_id").isin("1", "17", "13", "24"))
      // Convert to Integer
      val dfm02 = dfm01.columns.foldLeft(dfm01) { (tempDF, colName) => tempDF.withColumn(colName, col(colName).cast("integer"))}
      println("matchesNeeded")
      println(dfm02.show(5))

      val dfm = dfm02.select(
                            col("match_id"),
      ...
      val Stats = dfstats.withColumn("data", explode(col("data")))
        ...
        val dfs01 = Stats.filter(col("team_ID_").isin("1", "17", "13", "24")).filter(col("game_date").contains("2021"))
        // Filter  match_id with Match DataFrame and Stats DataFrame
        val listMatch = dfm.select("match_id").as[String].collect.toList
        val dfs02 = dfs01.filter(col("game_id").isin(listMatch: _*))
        
        // Convert Column Types
        val integerColumns = Seq(
                                "ast", "blk", "dreb", "fg3a", "fg3m", "fga", "fgm", "fta", "ftm", "home_team_id", "home_team_score", "game_id", "period", "season",
        ...
        // Print                                     
        println("StatsNeeded")
        println(dfs.select("team_ID_", "game_id","player_id", "player_first_name", "player_team_id", "pts","home_team_id", "home_team_score","visitor_team_id","visitor_team_score" ).show())
        ...
        val stats = dfs.join(df_new, dfs("game_id") === df_new("game_id")).drop("home_team_id", "home_team_score","visitor_team_id","visitor_team_score").drop(df_new("game_id"))
        print(stats.select("team_ID_", "game_id","player_id", "player_first_name", "player_team_id","team_id", "score").show(1))


        // Print Matches Data
        val matches = dfm.join(dfTeamsNeded.select("id", "full_name"), dfm("team_ID_") === dfTeamsNeded("id")).drop("id","where")
